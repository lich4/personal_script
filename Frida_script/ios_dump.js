...
function dumpMemory(base, size, offset) {
    base = ptr(base);
    var newmodpath = getDocumentDir() + "/mem.dmp";
    if(!access(allocStr(newmodpath),0)){
        remove(allocStr(newmodpath));
    }
    var fmodule = open(newmodpath, O_CREAT | O_WRONLY, 0);
    if (offset) lseek(fmodule, offset, SEEK_SET);
    write(fmodule, base, size);
    close(fmodule);
    console.log('dump to ' + newmodpath);
}
