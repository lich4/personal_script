...
def addobjcref():
    ...
    :return: nothing
    """
    dbfilename = get_root_filename() + '.xref'
    if not os.path.exists(dbfilename):
        cx = sqlite3.connect(dbfilename)
        cu = cx.cursor()
        cu.execute('create table xref (i integer primary key, x integer, y integer, z integer, k text)')
        analyze(cu)
        cx.commit()
        cu.execute('select count (*) from xref')
        count, = cu.fetchone()
        cu.close()
        cx.close()
    ...
    while True:
        ...
        if r is None:
            break
        i, x, y, z, k = r
        pool.putRequest(threadpool.WorkRequest(addxref, [x, y, z, i, k]))
    print 'Database ok'
    pool.wait()
    cu.close()
    cx.close()
