...
public class SourceTool extends JPanel {
    ...
    public boolean showSourceForLocation(Location locn) {
       ...
       String desc = locn.virtualMachine().description().toLowerCase();
       String dexpath = "";//dex|odex path
       if(desc.indexOf("jvm") != -1){//java virutalmachine
           ByteCodeModel model = new ByteCodeModel(locn.declaringType() ,locn.method(), locn.codeIndex());
           list.setModel(model);
           list.setSelectedIndex(model.selectedline);
       }
       else if(desc.indexOf("android") != -1){//dalvik virtual machine
           DalvikByteCodeModel model = new DalvikByteCodeModel(new File(dexpath) ,locn.method(), locn.codeIndex());
           list.setModel(model);
           list.setSelectedIndex(model.selectedline);
       }
